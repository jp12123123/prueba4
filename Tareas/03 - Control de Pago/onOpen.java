...
//-----------------------------Disabled-----------
Textbox comp_TXT_importeHistorico=(Textbox) sup.getInstanciaComponente("comp_TXT_importeHistorico");
Textbox comp_TXT_nroRecibo=(Textbox) sup.getInstanciaComponente("comp_TXT_nroRecibo");
Textbox comp_TXT_fechaPago=(Textbox) sup.getInstanciaComponente("comp_TXT_fechaPago");
...
//String tramitesss = comp_CMB_tramite.getValue();

//comp_CMB_tramite.setValue(tramitesss);
//comp_CMB_tramite.setSelectedIndex(0);
...
Div dis_realizoPago=(Div) sup.getInstanciaComponente("dis_realizoPago");
Div dis_clientePago=(Div) sup.getInstanciaComponente("dis_clientePago");
Div dis_fechaPago=(Div) sup.getInstanciaComponente("dis_fechaPago");
dis_realizoPago.setVisible(false);
dis_clientePago.setVisible(false);
dis_fechaPago.setVisible(false);

comp_TXT_fechaPago.setReadonly(true);
comp_TXT_fechaPago.setStyle("background-color: white;");
...
Div dis_realizoPago=(Div) sup.getInstanciaComponente("dis_realizoPago");
dis_realizoPago.setVisible(true);

/*try{
    //Messagebox.show("Antes de la función");
    java.util.List parametrosfnComprobacionPago = new java.util.ArrayList();
        parametrosfnComprobacionPago.add(comp_TXT_nroOBL.getValue().trim()); //OBL_ID / String
    new modelos.builder.eventos.ModeloEventos().ejecutarFuncion("fnComprobacionPago",parametrosfnComprobacionPago);
    Map return29map=(Map) ((org.zkoss.zul.Window) org.zkoss.zk.ui.Executions.getCurrent().getDesktop().getAttribute("winBuilder")).getAttribute("return29map");
    //Messagebox.show("Después de la función");
    
    if(return29map.get("items").size()>0){
        //Messagebox.show("fecha: "+return29map.get("items").get(0).get("fecha_cobro").toString());    
        comp_TXT_fechaPago.setValue(return29map.get("items").get(0).get("fecha_cobro").toString());
        comp_CHK_realizoPago.setChecked(true);
        comp_TXT_importeHistorico.setValue(return29map.get("items").get(0).get("obl_imp_original").toString());
		comp_TXT_nroRecibo.setValue(return29map.get("items").get(0).get("obl_nro_factura").toString());
		
        dis_clientePago.setVisible(true);
        dis_fechaPago.setVisible(true);
    } else {
         Messagebox.show("El usuario no ha pagado la factura", new Messagebox.Button[0], null);
    }
} catch (Exception e) {
    	Messagebox.show("Web Service de comprobación de pago no disponible","Advertencia",null,null,null);
    Logger.getLogger(modelos.builder.eventos.ModeloEventos.class.getName()).log(Level.SEVERE, null, e/*parametro de la exepcion);

}*/

comp_TXT_importeHistorico.setReadonly(true);
comp_TXT_importeHistorico.setStyle("background-color: white;");


comp_TXT_nroRecibo.setReadonly(true);
comp_TXT_nroRecibo.setStyle("background-color: white;");
