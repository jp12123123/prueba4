...
Textbox txtDni = (Textbox) sup.getInstanciaComponente("comp_TXT_dniTramitente2");
String value = txtDni.getValue().trim();
...
//PERSONA FISICA - VALIDA DNI
    ...
    String orden = txtDni.getValue(); // obtiene el el valor de la variable. Cambiar comp_TXT_bdOrden por la variable
    ...
    if (!matcherOrden.matches()) {
        Messagebox.show("Por favor, ingrese correctamente el DNI del segundo Transmitente/Hipotecario", "Advertencia", null, null, null);
        txtDni.setValue("-");
    }
}
...
public void validarCUIT(){
    ...
    String cuit = "";
        ...
        if (txtDni.getValue().trim().equals("-")) {
            Messagebox.show("Por favor, cargue el CUIT del segundo Transmietente/Hipotecario", "Advertencia", null, null, null);
