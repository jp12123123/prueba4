CREATE DEFINER=`pjuarez`@`localhost` PROCEDURE `grabaCUT`(ticket int)
BEGIN
...
and di1.name = 'comp_TXT_CUT_CLI_NOMBRE')  as 'CUT_CLI_NOMBRE',
(select CAST(di1.clobValue as char(20))
from bonita.data_instance di1 
where di1.containerId in (
                                               select di.containerId 
...
and di1.name = 'comp_TXT_CUT_CLI_TYPE')  as 'CUT_CLI_TYPE',
(select CAST(di1.clobValue as char(20))
from bonita.data_instance di1 
where di1.containerId in (
                                               select di.containerId 
...
and di1.name = 'comodin_texto_2')  as 'CUT_PH_CAL_EDIFICIO',
(select CAST(di1.clobValue as UNSIGNED)
from bonita.data_instance di1 
where di1.containerId in (
                                               select di.containerId 
